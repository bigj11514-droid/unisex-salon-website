document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll("[data-package-list]").forEach((list) => {
    list.innerHTML = packages[list.dataset.packageList]
      .map(
        (item, index) =>
          `<article class="package-card" data-package-service="${item.name}"><div class="package-image"><img src="${item.image}" alt="${item.name} service"><span>0${index + 1}</span></div><div class="package-copy"><div class="package-title"><div><h3>${item.name}</h3><p>${item.summary}</p></div><strong>${item.price}</strong></div><p class="package-details">${item.details}</p><div class="package-products"><b>Products used</b><ul>${item.products.map((product) => `<li>${product}</li>`).join("")}</ul></div><span class="package-duration">${item.duration}</span><a href="#booking" class="package-link" data-book-package="${item.name}">Book this service ↗</a></div></article>`,
      )
      .join("");
  });
  document
